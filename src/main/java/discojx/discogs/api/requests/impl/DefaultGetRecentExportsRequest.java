...
public class DefaultGetRecentExportsRequest extends AbstractRequest
    ...
    public CompletableFuture<EntityResponseWrapper<JsonNode>> executeAsync() {
        return CompletableFuture.supplyAsync(() -> {
            ...
            JsonNode getRecentExports;
            try {
                getRecentExports = JsonUtils.DefaultObjectMapperHolder.mapper.readTree(response.getEntity().getContent());
            } catch (IOException e) {
                throw new CompletionException(e);
            }

            return new EntityResponseWrapper<>(response, getRecentExports);
        });
    }
}
